"""
PHOENIX Strategy — Hedged Maker Market-Making with Spike Detection

Renamed from enhanced_spike.py (Feb 18, 2026).
This is the live trading strategy engine used by run_paper_bot.py.

PHOENIX = Paired Hedged Observation-driven Entry with eXecution INtelligence.

Core Logic:
    1. Detect BTC spike via EWMA on 60Hz Binance data
    2. PHOENIX: buy expensive side (maker bid at ask - offset)
    3. HEDGE: buy cheap side (maker bid, pair cost < $0.98)
    4. Hold to resolution — hedged pairs guarantee profit

Backward compatibility: EnhancedSpikeStrategy is aliased to PhoenixStrategy.

Usage:
    strategy = PhoenixStrategy(base_size=25, fade_mode=True, entry_offset=0.02)
...
if TYPE_CHECKING:
    from src.strategies.regime_threshold import RegimeAdaptiveThreshold
...
class EnhancedSpikeStrategy:
    ...
    def __init__(
        ...
        time_stop_seconds: float = 30.0,  # Matches TRADING_CONFIGS.AGGRESSIVE
        # Adaptive threshold (REGIME self-calibrating, duck-typed interface)
        ou_adaptive_threshold: Optional["RegimeAdaptiveThreshold"] = None,
        ...
        # Skip new entries during UTC hours where PHOENIX accuracy drops
        ...
        # PATIENT HEDGE (Feb 18, 2026)
        # Fixed $0.04 bid on cheap side. Fills when we predict correctly (resolution = expensive side).
        # PC96 safety: only hedge when fill_price + patient_bid <= max_pair_cost
        patient_hedge: bool = False,
        patient_bid: float = 0.04,
        max_pair_cost: float = 0.96,
        # ENTRY TIMING (Feb 18, 2026) — match backtest entry_start_secs
        # Only enter when time_remaining <= entry_start_secs (wait for last N seconds of market)
        entry_start_secs: float = 900.0,  # Default: no delay (900s = full 15-min market)
        # SPIKE COOLDOWN (Feb 18, 2026) — match backtest cooldown_secs
        spike_cooldown_secs: float = 0.0,  # Minimum seconds between entries
        # MAX FILL PRICE CAP (Feb 18, 2026) — skip entries above this price
        max_fill_price: float = 1.0,  # Default: no cap
        # PER-MARKET CAPITAL CAP (Feb 18, 2026) — track actual $ deployed per market
        max_capital_fraction: float = 0.50,  # Cap at 50% of current balance
    ):
        ...
        # Adaptive threshold — REGIME self-calibrating (replaces fixed threshold when set)
        ...
        # PATIENT HEDGE (Feb 18, 2026)
        # Fixed bid on cheap side instead of dynamic (max_pair_cost - fill_price)
        self.patient_hedge = patient_hedge
        self.patient_bid = patient_bid
        self.max_pair_cost = max_pair_cost

        # ENTRY TIMING (Feb 18, 2026) — match backtest entry_start_secs
        self.entry_start_secs = entry_start_secs
        # SPIKE COOLDOWN (Feb 18, 2026)
        self.spike_cooldown_secs = spike_cooldown_secs
        self._last_entry_time = 0.0  # Timestamp of last entry fill

        # MAX FILL PRICE CAP (Feb 18, 2026) — from backtest line 420
        self.max_fill_price = max_fill_price

        # PER-MARKET CAPITAL TRACKING (Feb 18, 2026) — from backtest lines 367-454
        # Tracks actual $ deployed per market, caps at max_capital_fraction of balance
        self.max_capital_fraction = max_capital_fraction
        self._market_capital_deployed = 0.0  # Actual $ cost in current market
    ...
    def detect_spike(self, binance_price: float) -> Tuple[Optional[str], float]:
        ...
        Uses REGIME adaptive threshold if ou_adaptive_threshold is set,
    ...
    def get_current_threshold(self) -> float:
        """Get current spike threshold (REGIME adaptive or fixed)."""
        if self.ou_adaptive_threshold is not None:
    ...
    def record_market_capital(self, fill_price: float, shares: int) -> None:
        """Track actual $ deployed in current market (for capital cap)."""
        cost = fill_price * shares
        self._market_capital_deployed += cost
        logger.info(
            f"[CAPITAL] Deployed ${cost:.2f} in market "
            f"(total: ${self._market_capital_deployed:.2f})"
        )

    def get_affordable_shares(self, fill_price: float, base_shares: int, current_balance: float) -> int:
        """Cap share count by per-market capital limit (backtest lines 444-454).

        Returns min(base_shares, max_affordable) based on remaining capital budget.
        """
        max_per_market = current_balance * self.max_capital_fraction
        remaining = max_per_market - self._market_capital_deployed
        if remaining <= 0:
            return 0
        max_affordable = int(remaining / fill_price) if fill_price > 0 else 0
        capped = min(base_shares, max_affordable)
        if capped < base_shares:
            logger.info(
                f"[CAPITAL] Shares capped: {base_shares} → {capped} "
                f"(remaining: ${remaining:.2f}/{max_per_market:.2f})"
            )
        return capped
    ...
    def get_quotes(
        ...
        # PHOENIX FIX: Allow hedge quotes through even when time is low
        has_unhedged_position = self.fade_mode and s.up_shares != s.down_shares
        if time_remaining < self.min_time_remaining and s.first_fill_side is None:
            if has_unhedged_position:
                # Time is short but we MUST hedge — generate hedge-only quotes
                quotes = []
                hedge_quote = self._generate_fade_hedge_quote(up_ask, down_ask)
                if hedge_quote:
                    quotes.append(hedge_quote)
                return quotes
            logger.debug(f"[ENHSPIKE] Skipping NEW ENTRY: {time_remaining:.0f}s remaining < {self.min_time_remaining:.0f}s min")
            return []

        # ENTRY START GATE (Feb 18, 2026) — match backtest entry_start_secs
        # Don't place NEW ENTRIES if too early in market (wait for last N seconds)
        if time_remaining > self.entry_start_secs and s.first_fill_side is None:
            if has_unhedged_position:
                quotes = []
                hedge_quote = self._generate_fade_hedge_quote(up_ask, down_ask)
                if hedge_quote:
                    quotes.append(hedge_quote)
                return quotes
            logger.debug(f"[ENHSPIKE] Skipping NEW ENTRY: {time_remaining:.0f}s > {self.entry_start_secs:.0f}s entry_start (too early)")
            return []

        # HOUR-OF-DAY FILTER (Feb 9, 2026 - Loser Analysis finding)
        # Skip new entries during bad UTC hours where PHOENIX accuracy drops
        # Only blocks new entries, NOT exit logic or hedge quotes
        # PHOENIX FIX: Allow hedge quotes through even during skip hours
        has_unhedged_position = self.fade_mode and s.up_shares != s.down_shares
        if self.skip_utc_hours and s.first_fill_side is None and not has_unhedged_position:
            ...
            if current_hour_utc in self.skip_utc_hours:
                ...
                return []
        # During skip hours with unhedged position: only generate hedge, no entries
        if self.skip_utc_hours and has_unhedged_position:
            current_hour_utc = datetime.now(timezone.utc).hour
            if current_hour_utc in self.skip_utc_hours:
                quotes = []
                hedge_quote = self._generate_fade_hedge_quote(up_ask, down_ask)
        ...
        if binance_price is not None:
            ...
            # PHOENIX FIX: Allow hedge quotes through when we have an unhedged position
        ...
        # PHOENIX MODE: Skip time-stop entirely — hold to resolution
        ...
        if s.first_fill_side is None:
            ...
            # =================================================================
            # PHOENIX MODE: Entries handled ONLY by SPIKE_EVENT handler (run_paper_bot.py)
            # get_quotes() only generates HEDGE quotes for existing positions.
            # This eliminates duplicate entry paths with inconsistent gate logic.
            # =================================================================
            if self.fade_mode:
                quotes = []
                hedge_quote = self._generate_fade_hedge_quote(up_ask, down_ask)
            ...
            # =================================================================
            # NON-PHOENIX MODE: Original spike entry logic
            # =================================================================
        ...
        # PHASE 2: Entry filled - place hedge
            ...
            # PHOENIX MODE: Generate hedge on cheap side (PHOENIX hedging)
            if self.fade_mode:
                hedge_quote = self._generate_fade_hedge_quote(up_ask, down_ask)
                if hedge_quote:
                    quotes.append(hedge_quote)
    ...
    def _generate_fade_hedge_quote(
        self,
        up_ask: Optional[float],
        down_ask: Optional[float],
    ) -> Optional[dict]:
        """
        Generate a hedge quote for PHOENIX mode.

        PATIENT HEDGE (default): Fixed $0.04 bid on cheap side.
        - Fills when cheap side price touches $0.04 (rare in live, but guaranteed
          if cheap side resolves to $0 — i.e., we predicted correctly).
        - Safety: only place hedge if fill_price + patient_bid <= max_pair_cost ($0.96).
        - Entries above $0.92 are unhedgeable (pair cost would exceed $0.96).

        LEGACY (patient_hedge=False): Dynamic bid at min(deficit_ask, 0.98 - entry_avg).

        Returns a hedge quote dict or None if no hedge needed.
        """
        s = self.state

        # No hedge needed if position is balanced or empty
        if s.up_shares == s.down_shares:
            return None
        if s.up_shares == 0 and s.down_shares == 0:
            return None

        # Determine which side needs hedging (deficit side = cheap side)
        if s.up_shares > s.down_shares:
            deficit_side = "DOWN"
            deficit_ask = down_ask
            surplus_avg_price = s.up_avg_price
            deficit_needed = s.up_shares - s.down_shares
        else:
            deficit_side = "UP"
            deficit_ask = up_ask
            surplus_avg_price = s.down_avg_price
            deficit_needed = s.down_shares - s.up_shares

        if deficit_ask is None:
            return None

        if self.patient_hedge:
            # PATIENT HEDGE: fixed bid at self.patient_bid ($0.04)
            hedge_bid = self.patient_bid

            # Safety check: pair_cost = entry_avg + hedge_bid must be <= max_pair_cost
            pair_cost = surplus_avg_price + hedge_bid
            if pair_cost > self.max_pair_cost:
                logger.debug(
                    f"[PATIENT_HEDGE] SKIP: pair_cost=${pair_cost:.3f} > "
                    f"max=${self.max_pair_cost:.2f} (entry_avg=${surplus_avg_price:.3f})"
                )
                return None
        else:
            # LEGACY: dynamic hedge bid
            max_hedge_price = round(0.98 - surplus_avg_price, 2)
            if max_hedge_price <= 0.01:
                logger.debug(
                    f"[PHOENIX_HEDGE] SKIP: surplus_avg=${surplus_avg_price:.3f} too high, "
                    f"max_hedge=${max_hedge_price:.2f}"
                )
                return None
            hedge_bid = min(deficit_ask, max_hedge_price)
            hedge_bid = round(max(0.01, hedge_bid), 2)

        # Size: hedge the full imbalance (capped at base_size per quote)
        hedge_size = min(int(deficit_needed), self.base_size)
        if hedge_size <= 0:
            return None

        mode = "PATIENT" if self.patient_hedge else "PHOENIX"
        logger.info(
            f"[{mode}_HEDGE] {deficit_side} bid @ ${hedge_bid:.3f} x{hedge_size} "
            f"(deficit={deficit_needed:.0f}, "
            f"deficit_ask=${deficit_ask:.3f}, surplus_avg=${surplus_avg_price:.3f})"
        )
    ...
    def on_fill(self, side: str, price: float, size: int, order_id: Optional[str] = None) -> None:
        ...
        if s.first_fill_side is None:
            ...
            # PHOENIX MODE: Track fills but keep phase open for new entries.
            # Position shares are kept (accumulated), phase state is cleared.
            # CAP3 limits total entries per market.
            # Hedge fills also pass through here since first_fill_side is reset.
            if self.fade_mode:
                # Detect if this is a hedge fill (fills on deficit side)
                is_hedge_fill = (
                    (side_upper == "DOWN" and s.up_shares > s.down_shares) or
                    (side_upper == "UP" and s.down_shares > s.up_shares) or
                    (s.up_shares == s.down_shares and s.up_shares > 0)  # Just balanced
                )
                fill_type = "HEDGE" if is_hedge_fill else "ENTRY"
                pair_cost = s.up_avg_price + s.down_avg_price if s.up_shares > 0 and s.down_shares > 0 else 0
                hedged_pairs = min(s.up_shares, s.down_shares)
                logger.info(
                    f"[PHOENIX] {fill_type} filled: {side_upper} {size}@${price:.3f} | "
                    f"Pairs={hedged_pairs:.0f}, PairCost=${pair_cost:.3f}, "
                    f"Imbal={s.imbalance:+d} (entries={self._market_entry_count})"
                )
    ...
    def _check_completed_pairs(self) -> None:
        ...
        # PHOENIX MODE: No pair matching — positions resolve at market end
        if self.fade_mode:
            return
    ...
    def reset(self) -> None:
        ...
        self._market_capital_deployed = 0.0  # Reset per-market capital tracker
...
# PHOENIX alias (Feb 18, 2026) — canonical name going forward
PhoenixStrategy = EnhancedSpikeStrategy
PhoenixState = EnhancedSpikeState
PhoenixPhase = EnhancedSpikePhase

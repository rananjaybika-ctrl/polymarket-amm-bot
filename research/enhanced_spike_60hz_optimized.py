...
def detect_spikes_vectorized(btc_df: pd.DataFrame) -> pd.DataFrame:
    ...
    print("  Detecting spikes (vectorized)...")
    ...
    if CONFIG.adaptive_volatility:
        print("    Using ADAPTIVE volatility thresholds...")
        ...
        print(f"    Thresholds used: LOW={CONFIG.regime_thresholds['LOW']}%, MED={CONFIG.regime_thresholds['MEDIUM']}%, HIGH={CONFIG.regime_thresholds['HIGH']}%")
    else:
        # Fixed threshold (old behavior)
    ...
    return df[['timestamp_ms', 'price', 'spike_detected', 'spike_direction', 'spike_magnitude', 'regime']]
...
def main():
    ...
    print(f"  Adaptive Vol:     {'ON' if CONFIG.adaptive_volatility else 'OFF'}")
    if CONFIG.adaptive_volatility:

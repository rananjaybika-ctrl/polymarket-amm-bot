#!/usr/bin/env python3
"""
EWMA Spike Base Test - TS180 Configuration

The user's insight: With longer time-stops we can be more selective.
- TS30: Need volume to win (many trades, lower accuracy)
- TS180: Can be selective (fewer trades, higher accuracy)

EWMA spike base reduces signals, which may HURT TS30 but HELP TS180.

Test: FIXED vs EWMA_1000 on TS180_OLD config
"""
...
# =============================================================================
# TS180_OLD CONFIG (different from TS30)
# =============================================================================
TARGET_SHARES = 50
TIME_STOP_SECONDS = 180.0  # LONGER time-stop
MIN_TIME = 240.0  # time_stop + 60s buffer
DROP_MULTIPLIER = 0.50
DROP_INTERCEPT = 0.08
TARGET_PAIR_COST = 0.99
HIGH_ENTRY_THRESHOLD = 0.90
MIN_CYCLE_GAP_MS = 200  # SLOWER cycling for TS180
...
def simulate_market(btc_spikes: pd.DataFrame, obs_df: pd.DataFrame,
                    slug: str, resolution: str, method_name: str, dataset_name: str) -> List[TradeResult]:
    """Simulate trading on a single market with TS180 config."""
            ...
            while obs_idx < len(mdf):
                ...
                if pd.notna(loser_ask) and loser_ask <= loser_target:
                    ...
                    trades.append(TradeResult(
                        ...
                        spike_magnitude=spike_mag, dataset=dataset_name, method=method_name
                    ))
                    in_position = False
                    position_data = None
...
def load_dataset(dataset_key: str, datasets_config: dict):
    """Load a dataset."""
    ...
    print(f"    {dataset_key}: {hours:.1f}h, {len(valid_slugs)} markets")

    return btc_df, obs_df, res_map, hours


def run_test():
    """Run EWMA spike base comparison with TS180 on all OBI datasets."""
    load_ou_params()
    ...
    DATASETS = {
        ...
        "OOS9.1": {
            ...
            "obs_files": ["research/observer/grid_obs_oos9_1.csv"],
        },
    }
    ...
    methods = [
        ...
        ("EWMA_1200", lambda df: precompute_spikes_ewma(df, 1200)),  # Match 72-tick lookback
    ]

    all_results = []

    for dataset_key in DATASETS.keys():
        btc_df, obs_df, res_map, hours = load_dataset(dataset_key, DATASETS)
        if btc_df is None:
            continue

        for method_name, spike_func in methods:
            print(f"\n{'='*60}")
            print(f"Testing: {method_name} on {dataset_key} (TS180)")
            print(f"{'='*60}")
            ...
            for slug in tqdm(slugs, desc=f"  Markets", leave=False):
                ...
                trades = simulate_market(btc_spikes, obs_df, slug, resolution, method_name, dataset_key)
            ...
            if all_trades:
                ...
                print(f"  Trades: {total_trades}, $/hr: ${hourly_rate:.2f}, Win%: {win_rate:.1f}%, Passive: {passive_pct:.1f}%")

    # Summary
    print("\n" + "=" * 100)
    print("EWMA SPIKE BASE WITH TS180 - OBI DATASETS COMPARISON")
    print("=" * 100)
    print()
    print(f"{'Method':<12} {'Dataset':<10} {'Trades':>8} {'$/hr':>10} {'Win%':>8} {'Passive%':>10} {'$/Trade':>10}")
    print("-" * 100)

    for r in sorted(all_results, key=lambda x: (x['dataset'], x['method'])):
        print(f"{r['method']:<12} {r['dataset']:<10} {r['trades']:>8} ${r['hourly_rate']:>9.2f} "
              f"{r['win_rate']:>7.1f}% {r['passive_pct']:>9.1f}% ${r['pnl_per_trade']:>9.3f}")

    # Combined results
    print("\n" + "=" * 100)
    print("COMBINED OBI RESULTS (OOS7 + OOS8 + OOS9.1)")
    print("=" * 100)
    ...
    combined['pnl_per_trade'] = combined['pnl_net'] / combined['trades']

    print()
    print(f"{'Method':<12} {'Total Trades':>12} {'Total PnL':>12} {'Hours':>8} {'$/hr':>10} {'$/Trade':>10}")
    print("-" * 70)

    for _, row in combined.iterrows():
        print(f"{row['method']:<12} {row['trades']:>12} ${row['pnl_net']:>11.2f} {row['hours']:>7.1f}h "
              f"${row['hourly_rate']:>9.2f} ${row['pnl_per_trade']:>9.3f}")

    # Save results
    summary_path = Path("research/findings/data/ewma_spike_base_ts180_comparison.csv")

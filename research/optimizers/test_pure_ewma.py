...
"""
=============================================================================
PURE EWMA TEST - Everything EWMA (Spike + Threshold)
=============================================================================

Research question (Feb 3, 2026):
We switched spike detection from FIXED lookback to EWMA_1000. But threshold
calculation still uses OU-based params with rolling windows. Should we unify
to pure EWMA for consistency?

Current hybrid:
  │ Component       │ Method       │ Window                  │
  ├─────────────────┼──────────────┼─────────────────────────┤
  │ Spike magnitude │ EWMA_1000    │ 1000ms half-life        │
  │ Threshold       │ OU sigmoid   │ Calibrated from fit     │
  │ Vol estimate    │ EWMA         │ 300-tick half-life      │

This test (pure EWMA):
  │ Component       │ Method       │ Window                  │
  ├─────────────────┼──────────────┼─────────────────────────┤
  │ Spike magnitude │ EWMA_1000    │ 1000ms half-life        │
  │ Threshold       │ EWMA ratio   │ vol_short / vol_long    │
  │ Vol short       │ EWMA         │ 1000ms half-life        │
  │ Vol long        │ EWMA         │ 5000/10000ms half-life  │

Grid dimensions:
1. Threshold method: OU (baseline) vs EWMA (test)
2. EWMA vol_long halflife: 5000ms, 10000ms (if EWMA)
3. EWMA threshold scale: 0.3, 0.5, 0.7 (if EWMA)

Fixed params (from validated winner):
- Spike method: EWMA_1000
- Time-stop: 30s
...
Usage:
    python research/optimizers/test_pure_ewma.py
"""
...
# Pure EWMA threshold params (test)
# threshold = base * (1 + scale * (vol_short/vol_long - 1))
EWMA_VOL_SHORT_MS = 1000      # Short-term vol EWMA half-life (matches spike EWMA)
EWMA_VOL_LONG_MS_OPTIONS = [5000, 10000]  # Long-term baseline half-life options
EWMA_THRESHOLD_SCALE_OPTIONS = [0.3, 0.5, 0.7]  # How much threshold scales with vol ratio

# Threshold methods to test
THRESHOLD_METHODS = ["OU", "EWMA"]  # OU = baseline, EWMA = pure EWMA test
...
@dataclass
class TestConfig:
    """Configuration for threshold method test (OU vs pure EWMA)."""
    name: str
    ...
    # Threshold method (key test dimension)
    threshold_method: str = "OU"  # "OU" (baseline) or "EWMA" (pure EWMA)
    ewma_vol_long_ms: int = 5000  # Long-term vol EWMA half-life (if threshold_method="EWMA")
    ewma_threshold_scale: float = 0.5  # Threshold scaling factor (if threshold_method="EWMA")
...
# =============================================================================
# Generate configs: threshold_method × (ewma params if EWMA)
# =============================================================================
# 1 OU baseline + (2 vol_long × 3 scale) = 1 + 6 = 7 configs
CONFIGS = []
...
# 1. OU baseline (current production)
...
# 2. Pure EWMA variants (test)
for vol_long_ms in EWMA_VOL_LONG_MS_OPTIONS:
    for scale in EWMA_THRESHOLD_SCALE_OPTIONS:
        name = f"EWMA_L{vol_long_ms//1000}k_S{int(scale*10)}"  # e.g., EWMA_L5k_S5
        CONFIGS.append(TestConfig(
            name=name,
            time_stop_seconds=ts,
            drop_multiplier=mult,
            ...
            ewma_vol_long_ms=vol_long_ms,
            ewma_threshold_scale=scale,
            max_cycles=max_cycles,
            shares_per_cycle=shares_per,
            cycle_mode="SINGLE",
            direction_mode=dir_mode,
...
print(f"Generated {len(CONFIGS)} configs: 1 OU baseline + {len(EWMA_VOL_LONG_MS_OPTIONS) * len(EWMA_THRESHOLD_SCALE_OPTIONS)} EWMA variants")
...
def run_grid_search():
    ...
    all_results = []
    all_deep_metrics = []
    checkpoint_path = Path("research/findings/data/pure_ewma_test_checkpoint.csv")

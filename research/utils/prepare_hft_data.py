...
def add_spike_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # CANONICAL from TRADING_CONFIGS.py (Jan 27, 2026): 72 ticks = 1200ms at 60Hz
    SPIKE_LOOKBACK = 72
    SPIKE_THRESHOLD = 0.02

    df['price_change_72tick'] = df['binance_price'].pct_change(periods=SPIKE_LOOKBACK) * 100
    df['spike_magnitude'] = df['price_change_72tick'].abs()
    df['spike_detected'] = df['spike_magnitude'] >= SPIKE_THRESHOLD
    df['spike_direction'] = df['price_change_72tick'].apply(

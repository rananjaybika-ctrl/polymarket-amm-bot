#!/usr/bin/env python3
"""
PHOENIX 2D Sweep — Expensive Threshold x Max Pair Cost

=============================================================================
COPIED FROM: phoenix_threshold_comparison.py (validated execution engine)
Execution engine: IDENTICAL. Maker fills, 0ms delay, 0% fee.
Signal: LOCKED to REGIME_x1.0 (fast/slow vol crossover, self-calibrating)
=============================================================================

Sweeps expensive_threshold [0.65, 0.70, 0.75, 0.80, 0.85] against
max_pair_cost [0.96, 0.97, 0.98, 0.99] to find the combination that
maximizes hedge rate while maintaining meaningful profit per pair.

Key insight: lower threshold = more hedge room at same pair cost.
Entry $0.70 at PC96 gives max_hedge_bid=$0.26 vs $0.16 at $0.80.

Usage:
    python research/backtests/phoenix_threshold_pc_sweep.py --data all
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(description="PHOENIX 2D Sweep: Threshold x PairCost")
    parser.add_argument('--data', default='all',
                       help='Comma-separated datasets, "train", "validation", or "all"')
    parser.add_argument('--output', default='research/findings/data/phoenix_threshold_pc_sweep.csv')
    parser.add_argument('--checkpoint', default='research/findings/data/phoenix_threshold_pc_sweep_checkpoint.csv')
    args = parser.parse_args()

    print("=" * 80)
    print("PHOENIX 2D SWEEP: Expensive Threshold x Max Pair Cost")
    print("Signal: LOCKED to REGIME_x1.0 (fast/slow vol crossover)")
    print("=" * 80)
    ...
    # 2D SWEEP PARAMETERS
    # ========================================================================
    THRESHOLD_VALUES = [0.65, 0.70, 0.75, 0.80, 0.85]
    PAIR_COST_VALUES = [0.96, 0.97, 0.98, 0.99]

    sweep_configs = list(product(THRESHOLD_VALUES, PAIR_COST_VALUES))
    total_runs = len(sweep_configs) * len(datasets)

    print(f"Datasets: {datasets}")
    print(f"Expensive thresholds: {THRESHOLD_VALUES}")
    print(f"Max pair costs: {PAIR_COST_VALUES}")
    print(f"Total runs: {total_runs} ({len(sweep_configs)} configs x {len(datasets)} datasets)")
    print(f"\nSignal: REGIME_x1.0 (fast={REGIME_FAST_WINDOW}, slow={REGIME_SLOW_WINDOW})")
    print(f"Fixed: offset=0.02, shares=25, entry_window=300-120s, cooldown=10s")
    print(f"Hedge: NON-patient (max_pair_cost based), hedge_offset=0.02")
    ...
    for dataset_key in datasets:
        ...
        # Compute spikes ONCE per dataset (signal is locked to REGIME_x1.0)
        btc_spikes_df = precompute_spikes_ewma(
            btc_df, EWMA_HALFLIFE_MS,
            threshold_method="regime", k_multiplier=0.0,
            regime_scale=1.0
        )

        # Extract spike arrays (reused for all configs in this dataset)
        ...
        n_spikes = len(spike_ts_all)

        # Precompute market data ONCE (spikes don't change across configs)
        ...
        # Sweep threshold x pair_cost
        for exp_thresh, max_pc in sweep_configs:
            config_label = f"T{int(exp_thresh*100)}_PC{int(max_pc*100)}"
            pbar.set_description(f"{dataset_key}/{config_label}")

            config = PhoenixConfig(
                name=f"PHX_{config_label}",
                expensive_threshold=exp_thresh,
            ...
            metrics['expensive_threshold'] = exp_thresh
            metrics['max_pair_cost'] = max_pc
            metrics['config_label'] = config_label
            metrics['config_name'] = config.name
            metrics['dataset'] = dataset_key
            metrics['n_spikes'] = n_spikes
            # Compute max hedge bid for reference
            metrics['max_hedge_bid'] = round(max_pc - exp_thresh + config.entry_offset, 4)
    ...
    # ==========================================================================
    # COMPARISON SUMMARY — 2D HEATMAP
    # ==========================================================================
    if len(results_df) > 0:
        print("\n" + "=" * 80)
        print("2D SWEEP RESULTS: Expensive Threshold x Max Pair Cost")
        print("Signal: REGIME_x1.0 | Offset: 0.02 | Window: 300-120s")
        print("=" * 80)

        # ---- TABLE 1: Average $/hr ----
        print(f"\n--- AVG $/HR (across {len(datasets)} datasets) ---")
        header = f"{'Threshold':<10}" + "".join(f"{'PC'+str(int(pc*100)):>10}" for pc in PAIR_COST_VALUES)
        print(header)
        print("-" * (10 + 10 * len(PAIR_COST_VALUES)))
        for thresh in THRESHOLD_VALUES:
            row = f"T{int(thresh*100):<9}"
            for pc in PAIR_COST_VALUES:
                subset = results_df[(results_df['expensive_threshold'] == thresh) &
                                    (results_df['max_pair_cost'] == pc)]
                if len(subset) > 0:
                    val = subset['pnl_per_hr'].mean()
                    row += f"${val:>8.2f} "
                else:
                    row += f"{'N/A':>10}"
            print(row)

        # ---- TABLE 2: Average Hedge Rate ----
        print(f"\n--- AVG HEDGE RATE % (target: >80%) ---")
        header = f"{'Threshold':<10}" + "".join(f"{'PC'+str(int(pc*100)):>10}" for pc in PAIR_COST_VALUES)
        print(header)
        print("-" * (10 + 10 * len(PAIR_COST_VALUES)))
        for thresh in THRESHOLD_VALUES:
        ...
        # ---- TABLE 3: Average Unhedged % ----
        print(f"\n--- AVG UNHEDGED % (must be <20%) ---")
        header = f"{'Threshold':<10}" + "".join(f"{'PC'+str(int(pc*100)):>10}" for pc in PAIR_COST_VALUES)
        print(header)
        print("-" * (10 + 10 * len(PAIR_COST_VALUES)))
        for thresh in THRESHOLD_VALUES:
            ...
            for pc in PAIR_COST_VALUES:
                subset = results_df[(results_df['expensive_threshold'] == thresh) &
                                    (results_df['max_pair_cost'] == pc)]
                if len(subset) > 0:
                    val = subset['unhedged_pct'].mean()
                    marker = " *" if val <= 20 else ""
                    row += f"{val:>7.1f}%{marker} "
                else:
                    row += f"{'N/A':>10}"
            print(row)

        # ---- TABLE 4: Total Trades ----
        print(f"\n--- TOTAL TRADES (across all datasets) ---")
        header = f"{'Threshold':<10}" + "".join(f"{'PC'+str(int(pc*100)):>10}" for pc in PAIR_COST_VALUES)
        print(header)
        print("-" * (10 + 10 * len(PAIR_COST_VALUES)))
        for thresh in THRESHOLD_VALUES:
        ...
        for thresh in THRESHOLD_VALUES:
            ...
            for pc in PAIR_COST_VALUES:
                subset = results_df[(results_df['expensive_threshold'] == thresh) &
                                    (results_df['max_pair_cost'] == pc)]
                if len(subset) > 0:
                    val = subset['win_rate'].mean()
                    row += f"{val:>8.1f}% "
                else:
                    row += f"{'N/A':>10}"
            print(row)

        # ---- TOP 5 CONFIGS (by $/hr, with hedge constraint) ----
        print(f"\n{'='*80}")
        print("TOP 5 CONFIGS (hedge_rate >= 80% AND unhedged < 20%)")
        print("=" * 80)

        # Aggregate across datasets
        agg = results_df.groupby(['expensive_threshold', 'max_pair_cost']).agg({
            'pnl_per_hr': 'mean',
            'total_pnl': 'sum',
            'trades': 'sum',
            'win_rate': 'mean',
        ...
        # Filter: hedge_rate >= 80%
        qualified = agg[agg['hedge_rate'] >= 80.0].sort_values('pnl_per_hr', ascending=False)

        if len(qualified) == 0:
            print("\nNO CONFIG meets hedge_rate >= 80%. Showing top 5 by hedge_rate instead:")
            qualified = agg.sort_values('hedge_rate', ascending=False).head(5)
        else:
            qualified = qualified.head(5)

        print(f"\n{'Config':<12} {'$/hr':>8} {'TotalPnL':>10} {'Trades':>7} {'WR%':>6} "
              f"{'Hedge%':>7} {'Unhgd%':>7} {'AvgPC':>7}")
        print("-" * 72)
        for _, row in qualified.iterrows():
            label = f"T{int(row['expensive_threshold']*100)}/PC{int(row['max_pair_cost']*100)}"
            print(f"{label:<12} ${row['pnl_per_hr']:>6.2f} ${row['total_pnl']:>9.2f} "
                  f"{int(row['trades']):>7} {row['win_rate']:>5.1f}% {row['hedge_rate']:>6.1f}% "
                  f"{row['unhedged_pct']:>6.1f}% ${row['avg_pair_cost']:>.4f}")

        # ---- TOP 5 BY PNL (no constraint) ----
        print(f"\n{'='*80}")
        print("TOP 5 CONFIGS (by $/hr, NO hedge constraint)")
        print("=" * 80)
        top_pnl = agg.sort_values('pnl_per_hr', ascending=False).head(5)
        print(f"\n{'Config':<12} {'$/hr':>8} {'TotalPnL':>10} {'Trades':>7} {'WR%':>6} "
              f"{'Hedge%':>7} {'Unhgd%':>7} {'AvgPC':>7}")
        print("-" * 72)
        ...
        # ---- PER-DATASET BREAKDOWN for best qualified config ----
        if len(qualified) > 0:
            best = qualified.iloc[0]
            best_thresh = best['expensive_threshold']
            best_pc = best['max_pair_cost']
            best_label = f"T{int(best_thresh*100)}/PC{int(best_pc*100)}"

            # Also show the current baseline (T80/PC96) for comparison
            baseline_thresh, baseline_pc = 0.80, 0.96

            print(f"\n{'='*80}")
            print(f"PER-DATASET: {best_label} (BEST QUALIFIED) vs T80/PC96 (CURRENT)")
            print("=" * 80)
            print(f"\n{'Dataset':<12} {'T80/PC96 $/hr':>14} {f'{best_label} $/hr':>14} {'Delta':>8} "
                  f"{'Base HR%':>9} {'Best HR%':>9} {'Base Trd':>9} {'Best Trd':>9}")
            print("-" * 95)

            for ds in datasets:
                base_row = results_df[(results_df['dataset'] == ds) &
                                       (results_df['expensive_threshold'] == baseline_thresh) &
                                       (results_df['max_pair_cost'] == baseline_pc)]
                best_row = results_df[(results_df['dataset'] == ds) &
                                       (results_df['expensive_threshold'] == best_thresh) &
                                       (results_df['max_pair_cost'] == best_pc)]
                if len(base_row) == 0 or len(best_row) == 0:
                    continue
                b_pnl = base_row.iloc[0]['pnl_per_hr']
                w_pnl = best_row.iloc[0]['pnl_per_hr']
                delta = w_pnl - b_pnl
                b_hr = base_row.iloc[0]['hedge_rate']
                w_hr = best_row.iloc[0]['hedge_rate']
                b_trades = int(base_row.iloc[0]['trades'])
                w_trades = int(best_row.iloc[0]['trades'])
                winner = "NEW" if delta > 0 else "BASE"
                print(f"{ds:<12} ${b_pnl:>13.2f} ${w_pnl:>13.2f} ${delta:>+7.2f} "
                      f"{b_hr:>8.1f}% {w_hr:>8.1f}% {b_trades:>9} {w_trades:>9}  [{winner}]")

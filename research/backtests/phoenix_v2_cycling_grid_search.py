#!/usr/bin/env python3
"""
PHOENIX V2 Cycling Grid Search — Multiple Entries Per Market

=============================================================================
PHOENIX V2 CYCLING (Feb 18, 2026 — Session 4)
=============================================================================

COPIED FROM: phoenix_v1_grid_search.py (validated Session 3 deliverable)
CHANGES: Strategy logic ONLY — cycling support. Execution engine UNCHANGED.

Key difference from V1:
- V1: max 1-2 entries per market (single entry or double-down same-side)
- V2: up to max_entries_per_market independent entries on consecutive spikes
- Each entry independently hedges
- Capital constraint: total_shares * max_entry_price <= max_per_market
- No same-side restriction (each spike evaluated independently)

V1 Finding: hedge_offset has ZERO effect (max_pair_cost cap always binds).
V2: Removed hedge_offset from grid, hardcoded to 0.02.

Execution Engine: IDENTICAL to V1 (copied from aggressive_m_v2_grid_search.py)
...
Grid Parameters (focused on cycling + V1 winners):
- expensive_threshold: [0.75, 0.80]  (V1 validated: T80 best)
- entry_start_secs: [300]            (V1 validated: 300 >> 600)
- entry_end_secs: [180, 120]         (V1 validated: both viable)
- entry_offset: [0.01, 0.02, 0.03]
- decel_required: [False]            (cycling requires ND — decel kills multi-spike)
- max_pair_cost: [0.96]              (V1 validated: PC96 best)
- base_shares: [10, 15, 25]          (scaling test)
- max_entries_per_market: [1, 2, 3, 5, 99]  (cycling levels)

Usage:
    python research/backtests/phoenix_v2_cycling_grid_search.py --data train
    python research/backtests/phoenix_v2_cycling_grid_search.py --data validation
    python research/backtests/phoenix_v2_cycling_grid_search.py --data all
"""
...
# =============================================================================
# FAST MARKET SIMULATION — V2 CYCLING (numpy-optimized)
# =============================================================================
#
# ===== FUTURE IMPROVEMENT IDEAS (not yet tested) =====
#
# [IDEA 1] WIDER WINDOW WITH CYCLING: V1 showed W600 is bad for single-entry.
#   But with cycling, more time = more spikes = more entries. W600 might become
#   viable if per-entry PnL stays positive. Test W600 with cycling on.
#
# [IDEA 2] TAPERING SHARE SIZE: Entry 1 gets full base_shares, entry 2 gets
#   75%, entry 3 gets 50%. Later entries are lower conviction (signal already
#   fired multiple times — mean-reversion may be exhausted).
#
# [IDEA 3] PER-ENTRY COOLDOWN TUNING: Current 10s between spikes. Spikes
#   cluster at 15-25s gaps. Test cooldown_secs = [5, 10, 15, 20] as grid param.
#
# [IDEA 4] HEDGE-FIRST APPROACH: When expensive_ask is very high (>0.90),
#   buy cheap side first (it's cheap, < $0.10). Then wait for expensive side
#   to dip for entry. Locks in cheap hedge, takes directional risk on entry.
#
# [IDEA 5] DYNAMIC PAIR COST: First entry uses PC96 (conservative). Later
#   entries allow PC97 (more aggressive) since portfolio already has hedged
#   positions absorbing risk.
#
# [IDEA 6] CROSS-MARKET CAPITAL MANAGEMENT: With cycling, multiple markets
#   may be active simultaneously. Current per-market cap (50%) could be
#   lowered to 25-30% to spread risk across concurrent markets.
#
# [IDEA 7] DECEL AS ENTRY-QUALITY BOOST: Instead of requiring decel (which
#   kills cycling), use decel as a share-size multiplier: decel=True → 1.5x
#   shares, decel=False → 1.0x shares. Gets accuracy boost without filtering.
#
# ================================================================
def simulate_market_fast(
    ...
    """
    Fast PHOENIX V2 simulation — CYCLING: multiple independent entries per market.

    V1 difference: V1 allowed 1-2 entries (double-down, same side only).
    V2: up to max_entries_per_market independent entries on consecutive spikes.
    Each entry independently hedges. No same-side restriction.
    Capital constraint: total shares * max price <= max_per_market.

    Execution engine matches paper_trading.py (UNCHANGED from V1):
    - Maker fill: price-touch when ask <= our_bid, 0ms delay, 0% fee
    """
    ...
    trades = []
    entries = 0
    total_shares_deployed = 0  # Track across all entries for capital cap
    last_signal_ts = 0
    ...
    for si in range(len(spike_ts)):
        ...
        # Expensive side from current prices (V2: re-evaluate each spike independently)
        ...
        # V2: NO same-side restriction (each entry is independent)
        ...
        # Shares (V2: cap based on REMAINING capital in this market)
        remaining_capital = max_per_market - (total_shares_deployed * fill_price)
        if remaining_capital <= 0:
            break  # Market capital exhausted
        shares = min(config.base_shares, int(remaining_capital / fill_price)) if fill_price > 0 else 0
        if shares <= 0:
            break  # Can't afford any more shares

        entries += 1
        total_shares_deployed += shares
        ...
        trades.append(TradeResult(
            ...
            entry_number=entries,  # V2: which entry in this market
...
def generate_grid_configs(quick: bool = False) -> List[PhoenixConfig]:
    """
    V2 Grid: Focused on cycling variants around V1 winning parameters.

    V1 validated winners: T80, W300, PC96, ND (no decel — required for cycling).
    V1 dead parameter: hedge_offset (hardcoded to 0.02).

    V2 tests:
    - max_entries_per_market: [2, 3, 5, 99]  (NO 1 — already tested in V1)
    - base_shares: [10, 15, 25]  (scaling test — does larger size work with cycling?)
    - entry_offset: [0.01, 0.02, 0.03]  (V1 showed O1 best for raw PnL, O3 for risk-adj)
    - entry_end_secs: [180, 120]  (both viable in V1)
    - expensive_threshold: [0.75, 0.80]  (V1 validated both)
    - entry_start_secs: [300, 600]  (V1 showed 300 >> 600, but cycling may change this)
    """
    if quick:
        grid = {
            ...
            "entry_end_secs": [180],
            "entry_offset": [0.01, 0.03],
            "max_entries_per_market": [2, 5],
            "base_shares": [15],
        }
    else:
        grid = {
            "expensive_threshold": [0.75, 0.80],
            "entry_start_secs": [300, 600],   # Retest W600 — cycling may help
            "entry_end_secs": [180, 120],
            "entry_offset": [0.01, 0.02, 0.03],
            "max_entries_per_market": [2, 3, 5, 99],  # 99 = unlimited cycling
            "base_shares": [10, 15, 25],
        }

    # Fixed params from V1 validated winners
    fixed = {
        "decel_required": False,   # Cycling requires ND
        "hedge_offset": 0.02,      # Dead param (cap always binds)
        "max_pair_cost": 0.96,     # V1 validated best
    }
    ...
    for combo in product(*values):
        ...
        name_parts = [
            ...
            "ND",  # Always no-decel for cycling
            f"PC96",  # Always PC96
            f"S{params['base_shares']}",
            f"C{params['max_entries_per_market']}",  # Cycling level
        ]
...
def main():
    ...
    parser.add_argument('--output', default='research/findings/data/phoenix_v2_cycling_results.csv')
    parser.add_argument('--checkpoint', default='research/findings/data/phoenix_v2_cycling_checkpoint.csv')
    parser.add_argument('--quick', action='store_true', help='Quick test with reduced grid')
    args = parser.parse_args()

    print("=" * 80)
    print("PHOENIX V2 CYCLING GRID SEARCH (Feb 18, 2026 — Session 4)")
    print("=" * 80)

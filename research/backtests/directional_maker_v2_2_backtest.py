#!/usr/bin/env python3
"""
Directional Maker V2.2 Backtest — OBI Dropped, EMA Span Sweep

=============================================================================
COPIED FROM: directional_maker_v2_backtest.py (V2.1 validated)
MODIFIED: Removed OBI requirement (proven useless), removed gabagool,
          added EMA span sweep, added min_confidence sweep.
=============================================================================

Strategy:
  1. Predict winner side (BTC EMA crossover + optional velocity/OBI confirmer)
  2. MAKER bid at ask - offset on winner side → wait for fill
  3. Once winner fills, MAKER bid for hedge on loser side at
     (target_pair_cost - winner_fill_price)
  4. Baseline only (no spike response) — V2.0 proved baseline wins
  5. Last 90s: taker fallback for any unhedged position

Key changes from V2.1:
  - OBI no longer required — proven identical accuracy with/without (62.5%)
  - Gabagool removed — catastrophic on all datasets (-$2,295 total)
  - EMA span sweep: (200,600), (300,1800), (500,1200)
  - min_confidence sweep: c1 (EMA only) vs c2 (require confirmer)
  - ALL 6 datasets now eligible (not just 4 with OBI)

Usage:
    python research/backtests/directional_maker_v2_2_backtest.py --data OOS7
    python research/backtests/directional_maker_v2_2_backtest.py --data train
    python research/backtests/directional_maker_v2_2_backtest.py --data test
    python research/backtests/directional_maker_v2_2_backtest.py --data all
"""
...
class V2Config:
    name: str
    # Signal
    signal_mode: str = "ema_only"     # "ema_only" (OBI/gabagool removed)
    ema_short_span: int = 300
    ema_long_span: int = 1800
    min_confidence: int = 2           # 1=EMA only, 2=require 1 confirmer
    ...
    # Spike response — baseline only (V2.0 proven winner)
    spike_mode: str = "none"
    spike_threshold_cents: float = 0.03
    # Flip-specific (kept for compatibility, not used in baseline)
    flip_multiplier: float = 2.0
    flip_target_pair_cost: float = 0.99
    # Avg-down-specific (kept for compatibility, not used in baseline)
    avg_down_max_additions: int = 1
    # Timing
        ...
        # ── STATE: WINNER_FILLED ────────────────────────────────
            ...
            if max_hedge_price > 0:
                ...
                if loser_ask <= max_hedge_price and hedge_ask_was_above:
                    h_shares = max(POLY_MIN_SHARES, int(winner_shares * hedge_ratio))
                    h_cost = max_hedge_price * h_shares
                    # Polymarket min order value check
                    if h_cost < POLY_MIN_ORDER_VALUE:
                        h_shares = max(h_shares, int(POLY_MIN_ORDER_VALUE / max_hedge_price) + 1)
                        h_cost = max_hedge_price * h_shares
            ...
            if time_rem < config.taker_fallback_time:
                ...
                if taker_result is not None:
                    ...
                    # Polymarket min order value check
                    if h_cost < POLY_MIN_ORDER_VALUE:
                        h_shares = max(h_shares, int(POLY_MIN_ORDER_VALUE / h_price) + 1)
                        h_cost = h_price * h_shares
...
# STATUS: COPY VERBATIM from V1 + train/test split labels
# Train (60%, ~134h): IS+OOS2 + OOS7 + OOS9
# Test  (40%, ~89h):  OOS3+4 + OOS8 + OOS10
# ═══════════════════════════════════════════════════════════════
DATASETS = {
    "IS+OOS2": {
...
def run_backtest_with_session_stops(
    ...
    """
    Run V2.2 backtest with session-level stops and cross-market state.

    Signal determination per market:
    - BTC EMA crossover → base prediction (always layer 1)
    - Velocity confirmation → optional confirmer (layer 2)
    - OBI contrarian → optional confirmer if data available (layer 3)
    - min_confidence=1: trade on EMA alone (no confirmation needed)
    - min_confidence=2: require at least 1 confirmer (velocity OR OBI)
    """
    session_pnl = 0.0
    ...
    last_fill_ts = 0
    cooldown_skips = 0
    obi_skips = 0  # renamed: now tracks confidence skips (not OBI-specific)

    # Pre-compute OBI availability (used as optional confirmer, never required)
    ...
    for market_slug in sorted_markets:
        ...
        # ── 3. Signal determination (V2.2: OBI optional, gabagool removed) ──
        # Layer 1: BTC EMA crossover (always available)
        ...
        # Layer 3: OBI contrarian (optional — used if available, never required)
        ...
        # Confidence: count confirming layers
        layers = 1  # EMA base
        ...
        if layers < config.min_confidence:
            obi_skips += 1  # track as "confidence skip"
            continue
        ...
        if layers >= 3:
            ...
            confidence_level = "LOW"  # Only reached when min_confidence=1
...
# ═══════════════════════════════════════════════════════════════
# SECTION: generate_grid_configs() — V2.2 (OBI dropped, EMA sweep)
# STATUS: UPDATED — OBI removed, gabagool removed, EMA span sweep added.
# ═══════════════════════════════════════════════════════════════
def generate_grid_configs() -> List[V2Config]:
    """
    V2.2 grid: EMA span sweep × confidence × bid offset.

    3 EMA pairs × 2 confidence × 3 offsets = 18 configs.
    ALL run on ALL 6 datasets (no OBI requirement).

    Parameters varied:
    - ema_spans: (200,600), (300,1800), (500,1200)
    - min_confidence: 1 (EMA only), 2 (require confirmer)
    - bid_offset_cents: 0.01, 0.02, 0.03
    """
    configs = []
    ema_spans = [(200, 600), (300, 1800), (500, 1200)]
    min_confidences = [1, 2]
    ...
    for short, long in ema_spans:
        for min_conf in min_confidences:
            for offset in bid_offsets:
                o_tag = f"{int(offset * 100)}c"
                c_tag = f"c{min_conf}"
                configs.append(V2Config(
                    name=f"V22_E{short}_{long}_{c_tag}_off{o_tag}",
                    signal_mode="ema_only",
                    ema_short_span=short,
                    ema_long_span=long,
                    min_confidence=min_conf,
                    target_pair_cost=0.96,  # Fixed: V2.1 proved P96 > P98
...
def main():
    ...
    parser.add_argument('--output', default='research/findings/data/directional_maker_v2_2_results.csv')
    parser.add_argument('--checkpoint', default='research/findings/data/directional_maker_v2_2_checkpoint.csv')
    args = parser.parse_args()

    print("=" * 80)
    print("DIRECTIONAL MAKER V2.2 GRID SEARCH (Feb 11, 2026)")
    print("V2.2: OBI dropped (proven useless), gabagool removed, EMA span sweep.")
    print("Strategy: Winner-first maker + EMA crossover + optional confirmers")
    print("=" * 80)
    ...
    for c in all_configs[:4]:
        print(f"  - {c.name}: EMA({c.ema_short_span},{c.ema_long_span}), "
              f"min_conf={c.min_confidence}, offset={c.bid_offset_cents}")
    ...
    for dataset_key in datasets:
        ...
        # Check OBI availability (optional confirmer, never required)
        has_obi = 'up_imbalance' in obs_df.columns and 'down_imbalance' in obs_df.columns
        print(f"  OBI columns available: {has_obi} (optional confirmer)")

        # V2.2: ALL configs run on ALL datasets (no OBI filtering)
        ...
        for i, config in enumerate(tqdm(dataset_configs, desc=f"  {dataset_key}")):
            ...
            metrics['min_confidence'] = config.min_confidence
    ...
    if len(results_df) > 0:
        ...
        if len(train_df) > 0 and len(test_df) > 0:
            ...
            for rank, (cfg_name, train_pnl) in enumerate(train_ranked.head(5).items(), 1):
                ...
                test_acc = test_subset['signal_accuracy'].mean() if len(test_subset) > 0 else 0
                train_acc = train_df[train_df['config_name'] == cfg_name]['signal_accuracy'].mean()
                print(f"  #{rank} {cfg_name}:")
                print(f"    Train PnL: ${train_pnl:.2f} (acc: {train_acc:.1f}%)")
                print(f"    Test  PnL: ${test_pnl:.2f} (acc: {test_acc:.1f}%)")

        # Analysis by EMA span pair
        print("\n" + "=" * 80)
        print("ANALYSIS BY EMA SPAN PAIR")
        print("=" * 80)
        results_df['ema_pair'] = results_df['ema_short'].astype(str) + ',' + results_df['ema_long'].astype(str)
        by_ema = results_df.groupby('ema_pair').agg({
            'total_pnl': ['sum', 'mean'],
            'signal_accuracy': 'mean',
            'hedge_rate': 'mean',
            'total_trades': 'sum',
        ...
        by_ema = by_ema.sort_values(('total_pnl', 'sum'), ascending=False)
        print(by_ema.to_string())

        # Analysis by min_confidence
        print("\n" + "=" * 80)
        print("ANALYSIS BY MIN CONFIDENCE")
        print("=" * 80)
        by_conf = results_df.groupby('min_confidence').agg({
            'total_pnl': ['sum', 'mean'],

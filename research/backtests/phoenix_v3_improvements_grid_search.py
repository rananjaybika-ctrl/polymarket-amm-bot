#!/usr/bin/env python3
"""
PHOENIX V3 Improvements Grid Search — Bug Fixes + Ideas 2,3,7 + Panic Hedge

=============================================================================
PHOENIX V3 IMPROVEMENTS (Feb 18, 2026 — Session 4 continued)
=============================================================================

COPIED FROM: phoenix_v2_cycling_grid_search.py (validated V2 deliverable)
CHANGES: Strategy logic ONLY — improvements on top of V2. Execution engine UNCHANGED.

Bug fixes from V2:
- Capital tracking: track actual cost spent (not shares * current_price)

Improvements tested:
- [Idea 7] Decel boost: multiply shares by decel_boost when decel detected (1.0=off)
- [Idea 2] Taper factor: entry N gets base_shares * taper^(N-1) shares (1.0=off)
- [Idea 3] Cooldown tuning: cooldown_secs as grid parameter [5, 10, 15]
- [Panic hedge] For unhedged positions: at <60s remaining, relax pair cost to
  panic_max_pc (e.g., 1.00 or 1.05) to cap downside on losing trades.

Fixed parameters from V2 winner (T80_W300-120_O2_ND_PC96_S25_C99):
- expensive_threshold: 0.80
- entry_start_secs: 300, entry_end_secs: 120
- decel_required: False, hedge_offset: 0.02, max_pair_cost: 0.96

Execution Engine: IDENTICAL to V2/V1 (from aggressive_m_v2_grid_search.py)
...
Usage:
    python research/backtests/phoenix_v3_improvements_grid_search.py --data all
    python research/backtests/phoenix_v3_improvements_grid_search.py --data all --quick
"""
...
def main():
    ...
    parser.add_argument('--output', default='research/findings/data/phoenix_v3_improvements_results.csv')
    parser.add_argument('--checkpoint', default='research/findings/data/phoenix_v3_improvements_checkpoint.csv')
    parser.add_argument('--quick', action='store_true', help='Quick test with reduced grid')
    args = parser.parse_args()

    print("=" * 80)
    print("PHOENIX V3 IMPROVEMENTS GRID SEARCH (Feb 18, 2026)")
    print("=" * 80)

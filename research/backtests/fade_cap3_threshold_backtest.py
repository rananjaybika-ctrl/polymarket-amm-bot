...
class FVMMConfig:
    ...
    max_expensive_ask: float = 1.0  # Upper bound (1.0 = no cap)
...
# ═══════════════════════════════════════════════════════════════
def is_valid_fade_signal(spike_dir: str, velocity_bps: float, expensive_ask: float, time_remaining: float,
                         min_expensive_ask: float = 0.80, max_expensive_ask: float = 1.0) -> bool:
    """
    FADE signal filter based on research findings.
    - expensive_ask >= min_expensive_ask
    - expensive_ask <= max_expensive_ask
    - NOT(spike=DOWN AND velocity<0)
    - time_remaining >= 90s
    """
    if expensive_ask < min_expensive_ask:
        return False
    if expensive_ask > max_expensive_ask:
        return False
        ...
        if config.mode == "fade":
            ...
            while spike_idx < len(market_spikes):
                ...
                if not is_valid_fade_signal(spike_dir, velocity_bps, expensive_ask, time_rem,
                                            config.min_expensive_ask, config.max_expensive_ask):
                    spike_idx += 1
                    continue
...
def generate_grid_configs() -> List[FVMMConfig]:
    ...
    FADE CAP3 threshold comparison: min=0.80, max=0.85
    Tests the effect of capping expensive_ask at 0.85 (skip entries above $0.85).
    Baseline FADE_CAP3 (no cap) included for comparison.
    """
    configs = []

    # ─── FADE CAP3 baseline (min=0.80, no max cap) ───
    configs.append(FVMMConfig(
        name="FADE_CAP3_T80_NOMAX",
        mode="fade",
        entry_offset_cents=0.03,
        min_expensive_ask=0.80,
        max_expensive_ask=1.0,  # No upper cap
        max_entries_per_market=3,
        use_capital_constraint=False,
    ))

    # ─── FADE CAP3 with 0.80-0.85 band ───
    configs.append(FVMMConfig(
        name="FADE_CAP3_T80_85",
        mode="fade",
        entry_offset_cents=0.03,
        min_expensive_ask=0.80,
        max_expensive_ask=0.85,
        max_entries_per_market=3,
        use_capital_constraint=False,
    ))
...
def main():
    ...
    parser.add_argument('--output', default='research/findings/data/fade_cap3_threshold.csv')
    parser.add_argument('--checkpoint', default='research/findings/data/fade_cap3_threshold_checkpoint.csv')
    args = parser.parse_args()

    print("=" * 80)
    print("FADE CAP3 THRESHOLD COMPARISON (Feb 10, 2026)")
    print("Copied from: fair_value_mm_v3_backtest.py (validated execution engine)")
    print("=" * 80)

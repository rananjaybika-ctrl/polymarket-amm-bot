#!/usr/bin/env python3
"""
PHOENIX Main Backtest — Single-config validation across datasets

=============================================================================
PHOENIX MAIN BACKTEST (Feb 18, 2026)
=============================================================================

USE THIS FOR: Quick validation with PHOENIX winner config (no grid search)
FOR GRID SEARCH: Use phoenix_v3_improvements_grid_search.py

COPIED FROM: phoenix_v3_improvements_grid_search.py (validated V3 deliverable)
Execution engine: IDENTICAL to V3 grid search. Maker fills, 0ms delay, 0% fee.

Default config = PHOENIX winner (same as TRADING_CONFIGS.py PHOENIX):
...
Usage:
    python research/backtests/phoenix_main_backtest.py --data all
    python research/backtests/phoenix_main_backtest.py --data train
    python research/backtests/phoenix_main_backtest.py --data all --hours  # enable hour filter
"""
...
def main():
    ...
    parser.add_argument('--hedge-offset', type=float, default=0.02, help='Hedge offset from cheap ask (default 0.02)')
    parser.add_argument('--max-fill', type=float, default=1.0, help='Max fill price cap (default 1.0=no cap, 0.90=skip entries above $0.90)')
    parser.add_argument('--escalate-to', type=float, default=0.0, help='Escalating hedge: relax pair cost to this after --escalate-secs (0=disabled)')
    parser.add_argument('--escalate-secs', type=float, default=60.0, help='Seconds before escalating hedge bid (default 60)')
    parser.add_argument('--patient-hedge', action='store_true', help='Patient hedge: bid at fixed low price and let convergence fill')
    parser.add_argument('--patient-bid', type=float, default=0.0, help='Fixed hedge bid for patient mode (0=auto: $1/shares)')
    parser.add_argument('--resolution-fills', action='store_true', help='Use resolution-based hedge fills (for all modes, not just patient)')
    ...
    escalate_str = f", escalate={args.pair_cost}→{args.escalate_to} in {args.escalate_secs}s" if args.escalate_to > 0 else ""
    patient_str = " [PATIENT HEDGE MODE]" if args.patient_hedge else ""
    print(f"Config: offset={args.offset}, pair_cost={args.pair_cost}, shares={args.shares}, max_entries={args.max_entries}, hedge_offset={args.hedge_offset}{escalate_str}{patient_str}")
    ...
    config = PhoenixConfig(
        name=f"PHX_O{int(args.offset*100)}_{'PATIENT' if args.patient_hedge else f'PC{int(args.pair_cost*100)}'}{'-RES' if args.resolution_fills else ''}_HO{int(args.hedge_offset*100)}_S{args.shares}_C{args.max_entries}",
        expensive_threshold=0.80,
        ...
        cooldown_secs=10,
        panic_max_pc=0.0,
        escalate_pc_final=args.escalate_to,
        escalate_secs=args.escalate_secs,
        patient_hedge=args.patient_hedge,
        patient_bid=args.patient_bid,
        resolution_fills=args.resolution_fills,
    )
    configs = [config]
    print(f"\nConfig: {config.name}")

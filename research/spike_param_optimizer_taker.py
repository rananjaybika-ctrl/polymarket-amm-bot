...
class TradeResult:
    ...
    hedge_type: str  # 'passive', 'stoploss', 'resolution', 'pulled', 'aggressive'
    pair_cost: float
    pnl: float
...
def generate_param_grid(quick: bool = False, path: str = "both") -> List[OptConfig]:
    ...
    if quick:
        ...
        stop_losses = [None, 0.12, 0.15]  # Focused: OFF vs 12% (best) vs 15%
        ...
        entry_pull_timeouts = [3.0, 5.0, 7.0, 10.0, 15.0]
...
def simulate_market(spikes_df: pd.DataFrame, obs_df: pd.DataFrame,
    ...
    for _, spike_row in market_spikes.iterrows():
        ...
        # Calculate hedge fee (taker fee only for stoploss/aggressive exits)
        if hedge_type in ["stoploss", "aggressive"]:
        ...
        if hedge_ratio >= 1.0:
            ...
            if hedge_type == "resolution" and resolution != winner_side:
                # Wrong direction, no hedge completed
...
def main():
    ...
    if args.path == "path1":
        ...
        print(f"  Order Pull:   Testing ON and OFF (irrelevant for taker)")
